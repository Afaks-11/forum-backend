...
export const communityInvitationIdParamSchema = z.object({
	id: z.uuid("Invalid community invitation UUID format").openapi({
		description: "REQUIRED: The invitation identifier.",
	}),
});

// The name is checked against both the name and slug unique indices: the slug
// is generated by lowercasing the name, so two differently-cased names would
// otherwise collide on the slug constraint and produce a confusing error.
...
export const updateCommunitySchema = z
	.object({
		name: z
			...
			.optional()
			.openapi({
				description: "OPTIONAL: The new unique public display name.",
				example: "NodeJS-Mastery",
			}),
		slug: z
			.string()
			.trim()
			.min(2, "Community slug must be at least 2 characters")
			.regex(
				/^[a-z0-9-]+$/,
				"Slugs must be lowercase alphanumeric strings with hyphens",
			)
			.optional()
			.openapi({
				description: "OPTIONAL: The new unique lowercase URL-safe slug.",
				example: "typescript-developers",
			}),
		description: z
			.string()
